...
$goldenDir = Join-Path $repoRoot 'tests\golden\cesium-milk-truck\s04-default\1280x720'
...
function Invoke-Capture([string]$Config, [string]$OutputDir) {
    ...
    & $exe --headless --lock-camera --output $OutputDir
    if ($LASTEXITCODE -ne 0) {
        throw "RenderLab $Config --output exited $LASTEXITCODE."
    }
...
function Invoke-Compare([string]$Config, [string]$CandidateDir) {
    ...
    return Invoke-Native $compare @('--candidate', $CandidateDir, '--reference', $goldenDir)
}

function Invoke-ChannelSwap([string]$Config, [string]$CandidateDir) {
    $compare = Get-Exe $Config $compareName
    Write-Step "Proving a channel swap fails ($CandidateDir roughness vs golden base-color)"
    $code = Invoke-Native $compare @('--channel-swap', '--candidate', $CandidateDir, '--reference', $goldenDir)
    if ($code -ne 0) {
...
function Invoke-Verify([string]$Config) {
    if (-not (Test-Path (Join-Path $goldenDir 'gbuffer-base-color.png'))) {
        throw "Approved goldens are missing under $goldenDir"
    }

    $run1 = Join-Path $repoRoot "results\s16-$($Config.ToLowerInvariant())-run1"
    $run2 = Join-Path $repoRoot "results\s16-$($Config.ToLowerInvariant())-run2"
...
foreach ($config in $configurations) {
    switch ($Mode) {
        'Capture' {
            $output = if ($Candidate) { $Candidate } else { Join-Path $repoRoot "results\s16-$($config.ToLowerInvariant())" }
            Invoke-Capture $config $output
        }
        'Compare' {
            $candidateDir = if ($Candidate) { $Candidate } else { Join-Path $repoRoot "results\s16-$($config.ToLowerInvariant())" }
...
Write-Step "golden.ps1 $Mode passed."
