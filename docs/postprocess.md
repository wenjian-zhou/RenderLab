...
CLI (S3.2): `--exposure-ev <float>` — default 0, no range clamp; non-finite text
(`nan` / `inf` / overflow) is malformed input and exits 2. It affects the Final
present and `final.png` only; debug views and `.rlhdr` stay exposure-independent.
...
`TonemapCommon.ush:116`; UE uploads `ToAP1.Inverse()` instead
(`SceneManagement.cpp:155`), which agrees with the composed literal product to float
precision. Round-trip identity error is ~2.5e-10 (float64 reference).
...
| `PostProcessPass` (S3.2) | normal path | display-referred linear; hardware OETF once |
...
2. **Entry guard**: scene color is clamped to non-negative before exposure
   (`max(sceneColor, 0)`). UE's LUT shaper domain is strictly positive and its
   lighting output is non-negative, so this is identity on the real domain; it keeps
   stray negatives from reaching `log10`.
3. **Expand-gamut guard**: expansion is skipped when AP1 luminance <= 0. UE evaluates
   the chain only on its strictly positive LUT grid; per-pixel black would compute
   `0 / 0 = NaN` in `chroma = colorAP1 / luma`.
4. **Guarded toe/shoulder blend**: the blend selects toe at `t <= 0` and shoulder at
...
5. **fp32 evaluation**: UE evaluates the log-curve section in `half` and quantizes
...
- S3.2 landed the display-referred LDR golden: `--output-hdr` also renders
  `PostProcessPass` into a `SRGBA8_UNORM` dump target (same hardware OETF path as the
  presented back buffer) and writes `final.png` next to the `.rlhdr` in the same
  capture. `hdr-capture-metadata.json` is schema `v2` with the capture's
  `exposureEV` as a required, locked identity field (the approved golden pins EV 0);
  `RenderLabGoldenCompare --mode hdr` compares `final.png` with the S1.6 8-bit rules
  and tolerances alongside the `.rlhdr` float compare. `--output` / `golden.ps1`
  remain the GBuffer-only oracle and are unaffected by this contract.
- `scripts/ldr_stats.py` computes luminance statistics and adjacent-EV ratios over
  `final.png` captures. It is evidence tooling for the exposure sweep and highlight
  rolloff, not a regression gate.
...
- `scripts/postprocess_crosscheck.py` parses the UE 5.8.1 sources, the C++ contract,
  and the HLSL mirror, and asserts every matrix and scalar literal matches across all
  three (matrices compared against the UE literal transposed).
- `postprocess_ps.hlsl` is registered in `Shaders.cfg` and compiled by
  `renderlab_shaders` (SM 6.5), so the S3.1 temporary dxc smoke-compile entry is
  retired.
