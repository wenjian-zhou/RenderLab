...
- The HLSL mirror is smoke-compiled with the local dxc (a temporary pixel-shader
  entry under `out/` includes `postprocess.hlsli` and calls the chain) until S3.2
  adds the real `PostProcessPass` to `Shaders.cfg`.
